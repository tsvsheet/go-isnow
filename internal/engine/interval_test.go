...
func holdsCase(t *testing.T, pat PatternText, cases map[string]bool) {
	t.Helper()
	p := mustParse(t, pat)
...
func TestIntervalErrors(t *testing.T) {
	if _, err := Parse("-[90mn]"); !errors.Is(err, constants.ErrContext) {
		t.Fatalf("descending interval: %v", err)
	}
	if _, err := Parse("+[0mn]"); !errors.Is(err, constants.ErrRange) {
		t.Fatalf("zero interval: %v", err)
	}
	if _, err := Parse("+[99999999999999999999h]"); !errors.Is(err, constants.ErrRange) {
...
func TestBareWeekUnitIsNotAnInterval(t *testing.T) {
	...
	if _, err := Parse("+[3w] noon"); !errors.Is(err, constants.ErrContext) {
