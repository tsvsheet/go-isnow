...
var intervalUnits = map[unitName]intervalUnit{"s": unitSecond, "mn": unitMinute, "h": unitHour, "d": unitDay}
...
func containerFor(u intervalUnit, n count) container {
	p := u.minContainer()
	for p < cYear && count(nominalSeconds[p]/u.seconds()) < n {
		p++
	}
	return p
}
...
func (iv intervalSpec) holds(c instantCtx) bool {
	...
	pos := count(secondsInto(iv.container, c) / iv.unit.seconds())
...
func compileInterval(in rawIncr) (intervalSpec, error) {
	if in.isFromEnd {
		return intervalSpec{}, constants.ErrContext // a descending interval is meaningless
	}
	q := in.qtys[0]
	...
	return intervalSpec{unit: u, n: n, container: containerFor(u, n), text: fmt.Sprintf("+[%d%s]", int(n), q.unit)}, nil
}
