...
// separator joins the positional fields of a rendered group ('/' or ':').
...
func join(sl slots, sep separator, roles ...role) string {
...
func fieldText(sl slots, r role) string {
	if !sl[r].isAssigned || !sl[r].field.isPresent {
		return "*"
	}
	return renderField(r, sl[r].field)
}
...
func renderExact(r role, a rawAtom) []string {
	...
	return []string{pad(r, fieldValue(a.qtys[0].num))}
}
...
func renderEndpoint(r role, a rawAtom) string {
	...
	return pad(r, fieldValue(a.qtys[0].num))
}
...
func weekdayNamesOf(name symbolName) []string {
...
func pad(r role, v fieldValue) string {
	if r == roleYear {
...
func capitalize(w weekdayName) string {
...
func boundGroup(fields []boundField, sep separator, roles ...role) string {
	var parts []string
	for _, r := range roles {
...
func boundLookup(fields []boundField, r role) (fieldValue, bool) {
