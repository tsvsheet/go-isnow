...
func mustParse(t *testing.T, src PatternText) Pattern {
	t.Helper()
	p, err := Parse(src)
	if err != nil {
...
func TestIs(t *testing.T) {
	...
	if _, err := Is("25", time.Now()); !errors.Is(err, constants.ErrRange) {
		t.Fatalf("Is(bad) err = %v, want constants.ErrRange", err)
	}
}
