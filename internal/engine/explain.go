...
// phrase is the fixed English prefix a clause is rendered with.
type phrase string
...
func appendClause(out []string, sl slots, r role, prefix phrase) []string {
	text := fieldText(sl, r)
	if text == "*" {
		return out
	}
	return append(out, string(prefix)+text)
}
