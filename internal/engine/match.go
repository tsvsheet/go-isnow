...
func newCtx(at time.Time) instantCtx {
	y, m, d := at.Date()
	dim := daysInMonth(fieldValue(y), fieldValue(m), at.Location())
	return instantCtx{
...
func daysInMonth(year, month fieldValue, loc *time.Location) int {
	return time.Date(int(year), time.Month(month)+1, 0, 0, 0, 0, 0, loc).Day()
}
