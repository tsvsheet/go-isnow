...
// scanForward selects the direction a derivation scans: toward the future
// (Next) or the past (Prev).
type scanForward bool
...
func (p Pattern) deriveCtx(ctx context.Context, from time.Time, isForward scanForward) (time.Time, bool, error) {
	for day := truncateDay(from); !beyondHorizon(from, day, isForward); day = day.AddDate(0, 0, direction(isForward)) {
		if err := ctx.Err(); err != nil {
			return time.Time{}, false, err
		}
		if inst, ok := p.dayMatch(day, from, isForward); ok {
...
func direction(isForward scanForward) int {
	if isForward {
		return 1
	}
	return -1
...
func beyondHorizon(from, day time.Time, isForward scanForward) bool {
	if isForward {
...
func (p Pattern) dayMatch(day, from time.Time, isForward scanForward) (time.Time, bool) {
	...
	for _, h := range hours {
		for _, mi := range minutes {
			for _, s := range seconds {
				out = append(out, time.Date(y, m, d, int(h), int(mi), int(s), 0, loc))
			}
		}
	}
	return out
...
func (p Pattern) matching(r role, c instantCtx, lo, hi fieldValue) []fieldValue {
	var out []fieldValue
	for v := lo; v <= hi; v++ {
...
func (c instantCtx) with(r role, v fieldValue) instantCtx {
	switch r {
	case roleHour:
		c.hour = int(v)
	case roleMinute:
